...
LOCAL_CONFIG_PATH = Path("config/mart_restaurant_daily_sales.local.json")
    ...
    if running_in_glue():
        ...
        if not LOCAL_CONFIG_PATH.exists():
            raise FileNotFoundError(
                ...
                "Copy config/mart_restaurant_daily_sales.local.example.json to "
                "config/mart_restaurant_daily_sales.local.json and fill in the values."
            )
...
def main() -> None:
    ...
    print(f"Writing restaurant_daily_sales mart to: {mart_output_path}")
    ...
    restaurant_daily_sales_base_count = restaurant_daily_sales_base.count()

    print(f"restaurant_daily_sales_base row count: {restaurant_daily_sales_base_count}")
    ...
    unmatched_date_count = (
        restaurant_daily_sales_base.alias("sales")
        .join(
            date_dim.alias("dt"),
    ...
    mart_count = restaurant_daily_sales.count()
    ...
    max_daily_restaurant_net_revenue = (
        restaurant_daily_sales
        .select(F.max("net_revenue").alias("max_daily_restaurant_net_revenue"))
        .collect()[0]["max_daily_restaurant_net_revenue"]
    )
    ...
    print(f"Restaurant daily sales mart row count: {mart_count}")
    print(f"Distinct restaurants in restaurant_daily_sales mart: {distinct_restaurant_count}")
    print(f"Unmatched date_dim rows: {unmatched_date_count}")
    print(f"Total order count in restaurant_daily_sales mart: {total_order_count}")
    print(f"Total net revenue in restaurant_daily_sales mart: {total_net_revenue}")
    print(f"Max daily restaurant net revenue: {max_daily_restaurant_net_revenue}")
    print(f"Min order_date: {min_order_date}")
    print(f"Max order_date: {max_order_date}")

    if unmatched_date_count > 0:
        print(
            "WARNING: Some restaurant_daily_sales rows did not match date_dim_clean. "
            "Derived date attributes from order_date for unmatched rows. "
            f"unmatched_date_count={unmatched_date_count}"
        )

    if mart_count <= 0:
        raise ValueError("restaurant_daily_sales mart produced zero rows.")
    ...
    if total_net_revenue != source_total_net_revenue:
        raise ValueError(
            "Revenue mismatch between fact_order and restaurant_daily_sales mart. "
            f"fact_order_total={source_total_net_revenue}, "
            f"restaurant_daily_sales_total={total_net_revenue}"
        )

    source_order_count = (
        fact_order
        .select(F.countDistinct("order_id").alias("total"))
        .collect()[0]["total"]
    )

    if total_order_count != source_order_count:
        raise ValueError(
            "Order count mismatch between fact_order and restaurant_daily_sales mart. "
            f"fact_order_order_count={source_order_count}, "
    ...
    print(f"Successfully wrote restaurant_daily_sales mart to: {mart_output_path}")
