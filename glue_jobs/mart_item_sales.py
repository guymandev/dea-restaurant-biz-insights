...
LOCAL_CONFIG_PATH = Path("config/mart_item_sales.local.json")
    ...
    if running_in_glue():
        ...
        if not LOCAL_CONFIG_PATH.exists():
            raise FileNotFoundError(
                ...
                "Copy config/mart_item_sales.local.example.json to "
                "config/mart_item_sales.local.json and fill in the values."
            )
...
def main() -> None:
    ...
    print(f"Writing item_sales mart to: {mart_output_path}")
    ...
    item_sales = (
        fact_order_line
        .groupBy("item_category", "item_name")
        .agg(
    ...
    top_net_revenue_item = (
        item_sales
        .orderBy(F.col("net_revenue").desc())
        .select("item_category", "item_name", "net_revenue")
        .limit(1)
        .collect()
    )

    print(f"Item sales mart row count: {mart_count}")
    print(f"Distinct item/category combinations: {distinct_item_count}")
    print(f"Total source net revenue: {total_source_net_revenue}")
    print(f"Total item_sales net revenue: {total_mart_net_revenue}")
    print(f"Total source line item count: {total_source_line_item_count}")
    print(f"Total item_sales line item count: {total_mart_line_item_count}")
    print(f"Total source item quantity: {total_source_item_quantity}")
    print(f"Total item_sales item quantity: {total_mart_item_quantity}")

    if top_net_revenue_item:
        row = top_net_revenue_item[0]
        print(
    ...
    if mart_count <= 0:
        raise ValueError("item_sales mart produced zero rows.")

    if mart_count != distinct_item_count:
        raise ValueError(
            "item_sales should have one row per item_category + item_name. "
            f"mart_count={mart_count}, distinct_item_count={distinct_item_count}"
        )

    if total_mart_net_revenue != total_source_net_revenue:
        raise ValueError(
            "Revenue mismatch between fact_order_line and item_sales mart. "
            f"fact_order_line_total={total_source_net_revenue}, "
            f"item_sales_total={total_mart_net_revenue}"
        )

    if total_mart_line_item_count != total_source_line_item_count:
        raise ValueError(
            "Line item count mismatch between fact_order_line and item_sales mart. "
            f"fact_order_line_count={total_source_line_item_count}, "
            f"item_sales_line_item_count={total_mart_line_item_count}"
        )

    if total_mart_item_quantity != total_source_item_quantity:
        raise ValueError(
            "Item quantity mismatch between fact_order_line and item_sales mart. "
    ...
    print(f"Successfully wrote item_sales mart to: {mart_output_path}")
